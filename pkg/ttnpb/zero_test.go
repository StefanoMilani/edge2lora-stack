...
func TestFieldIsZero(t *testing.T) {
	for v, paths := range map[interface{ FieldIsZero(string) bool }][]string{
		...
		&EndDeviceClaimAuthenticationCode{}:      EndDeviceClaimAuthenticationCodeFieldPathsNested,
		(*EndDeviceClaimAuthenticationCode)(nil): EndDeviceClaimAuthenticationCodeFieldPathsNested,
