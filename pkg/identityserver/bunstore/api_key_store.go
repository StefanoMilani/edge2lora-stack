...
func (s *apiKeyStore) UpdateAPIKey(
	...
	// TODO NICK: Remove this and update UpdateAPIKey's usage with nil rights to call deleteMember
	// (https://github.com/TheThingsNetwork/lorawan-stack/issues/5587)
	if len(pb.Rights) == 0 && ttnpb.HasAnyField(fieldMask, "rights") {
		return nil, s.DeleteAPIKey(ctx, entityID, pb)
	}
...
func (s *apiKeyStore) DeleteAPIKey(
	...
	ctx, span := tracer.StartFromContext(ctx, "DeleteAPIKeys", trace.WithAttributes(
		attribute.String("entity_type", entityID.EntityType()),
		attribute.String("entity_id", entityID.IDString()),
	))
	defer span.End()
