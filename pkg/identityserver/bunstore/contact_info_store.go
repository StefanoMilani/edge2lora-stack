...
func (s *contactInfoStore) ValidateContactInfo(ctx context.Context, v *ttnpb.ContactInfoValidation) error {
	if len(v.GetContactInfo()) == 0 {
		return store.ErrValidationWithoutContactInfo.New()
	}
	contactInfo := v.GetContactInfo()[0]
	_, err := s.DB.NewUpdate().
		Model(&ContactInfo{}).
		Where("entity_type = ? AND entity_id = ?", v.GetEntity().EntityType(), v.GetEntity().IDString()).
		Where("contact_method = ? AND LOWER(value) = LOWER(?)", contactInfo.GetContactMethod(), contactInfo.GetValue()).
		Set("validated_at = ?", v.ExpiresAt.AsTime()).
		Exec(ctx)
	if err != nil {
		return storeutil.WrapDriverError(err)
	}

	if v.Entity.EntityType() == "user" &&
		ttnpb.ContactMethod(contactInfo.ContactMethod) == ttnpb.ContactMethod_CONTACT_METHOD_EMAIL {
		_, err = s.DB.NewUpdate().
			Model(&User{}).
...
func (s *contactInfoStore) ExpireValidation(ctx context.Context, v *ttnpb.ContactInfoValidation) error {
	_, err := s.DB.NewUpdate().
		Model(&ContactInfoValidation{}).
		Where("reference = ? AND token = ?", v.Id, v.Token).
